function[ qrsComplexes] = PrematureBeatEvaulation2( qrsComplexes )
...
RRI=4*250*qrsComplexes.RRInterval;
Rp=zeros(length(RRI),1);
Rn=zeros(length(RRI),1);
for p=2:length(RRI)-1
    Rp(p)=RRI(p-1);
    Rn(p)=RRI(p+1);
end
...
% end
l=find(ectopics==1);
x=[ qrsComplexes.QRSAmplitude(l) qrsComplexes.RRInterval(l)  qrsComplexes.PeakAngle(l)  hrChange(l) qrsComplexes.QRSInterval(l)]; 
[idx,C] = kmeans(x,2);
% v=find(idx==1);
% a=find(idx==2);
l1=length(qrsComplexes.QRSInterval(idx==1));
l2=length(qrsComplexes.QRSInterval(idx==2));
if l1<l2
    bound=l1;
else
    bound=l2;
end
m1=sort(qrsComplexes.QRSInterval(idx==1),'descend');
m2=sort(qrsComplexes.QRSInterval(idx==2),'descend');

if mean(m1(1:bound))> mean(m2(1:bound))
    la=l(idx==2);
    lv=l(idx==1);
else
    la=l(idx==1);
...
possibleAtrial=zeros(length(qrsComplexes.R),1);

possibleVentricular(lv)=true;
possibleAtrial(la)=true;

% %% COMPENSATORY PAUSE %% 
% RRI=4*250*qrsComplexes.RRInterval;
% dem=RRI(qrsComplexes.NoisyBeat==0 & ectopics==0);
% dem=(dem(dem>0 & dem<1300));
% dem=mean(dem);
% comp=zeros(length(RRI),1);
% for j =1:length(RRI)-1
%     if ectopics(j)==1
%         if ( (RRI(j) + RRI(j+1)) /dem) >=1.3
%           comp(j)=true;
%        end
%     end
% end
...
% 
% possibleVentricular(ectopics ==1 & single(qrsComplexes.QRSInterval) >= single(intervalTh) & qrsComplexes.P.StartPoint==1 &comp==true )=true;  
% possibleAtrial(ectopics ==1 & ~possibleVentricular &  single(qrsComplexes.QRSInterval) < single(intervalTh) & single(hR)> single(90) & comp==false )=true; 
% 
% 
...
% 
% uEVindex= (undefinedEctopicBeats==1 & comp==true & qrsComplexes.P.StartPoint==1 & single(qrsComplexes.QRSInterval) >= single(intervalTh)  );
