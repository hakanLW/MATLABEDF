function [ qrsComplexes,similarity ] = IntMorphBasedRecognition2( qrsComplexes,ecgSignal)

qsArea=cell(length(qrsComplexes.R),1);
area=zeros(length(qrsComplexes.R),1);
for q =1:length(qrsComplexes.R)
    qsArea{q}=ecgSignal(qrsComplexes.Q(q):qrsComplexes.S(q));
    area(q)=trapz(abs(qsArea{q}));
end
...
for o =1:length(similarity)
    if area(o)>=1.6*area(N)
          possibleVentricular(o)=true;
    elseif similarity(o)<=0.89 && ectopics(o)==true &&  area(o) > area(N)
        possibleVentricular(o)=true;   
    end
end

for p =2:length(possibleVentricular)-1
    if possibleVentricular(p-1)==true && possibleVentricular(p)==true
        if 1.5*area(p-1)<area(p)
            possibleVentricular(p-1)=false;
        end
    end
end
% %% P WAVE SIMILARITY %% 
